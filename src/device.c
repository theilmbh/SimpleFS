...
int dev_read(dev_t device, long int block_num, char *buf)
{
	...
	/* Read in block */
	size_t n = fread(buf, sizeof(char), BYESPERBLOCK, dev_fd)
	if(n != BYTESPERBLOCK){
...
int dev_write(dev_t device, int block_num, char *buf)
{
